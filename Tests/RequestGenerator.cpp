...
void RequestGenerator::createTree(const std::string& file) {
    ...
    while (f >> name) {
        if (name.front() == START_GENERATED ||
            name.front() == START_NON_GENERATED) {
            if (name.size() > 3 && name[0] == START_NON_GENERATED &&
                name[1] == STANDARD_FUNCTION) {
...
std::string RequestGenerator::implCommand(const std::string& s) {
    if (s.size() > 0) {
        ...
        } else if (s.front() == START_NON_GENERATED ||
                   s.front() == START_GENERATED) {
...
std::string RequestGenerator::getName(const std::string& s) {
    std::string ans = getLater();
    while (((Rand() % 3) != 0) || (not_generated_names_.find(to_lower(ans)) !=
                                   not_generated_names_.end())) {
